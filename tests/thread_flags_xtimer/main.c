...
int main(void)
{
    ...
    printf("You should see the message '+++ timeout XX +++' printed to the \n"
           "screen %i times, once every %i milliseconds\n\n",
    ...
    /* we consider the test successful, if the runtime was above 20 seconds */
    uint32_t runtime = xtimer_now_usec() - now;
    if (runtime > RUNTIME) {
