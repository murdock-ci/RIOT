...
int main(void)
{
    ...
    while (1) {
        ...
        temperature = bmp180_read_temperature(&dev);

        /* Get pressure in Pa */
        pressure = bmp180_read_pressure(&dev);

        /* Get pressure at sealevel in Pa */
        pressure_0 = bmp180_sealevel_pressure(&dev, (int16_t)TEST_ALTITUDE);

        /* Get altitude in meters */
        altitude = bmp180_altitude(&dev, pressure_0);
